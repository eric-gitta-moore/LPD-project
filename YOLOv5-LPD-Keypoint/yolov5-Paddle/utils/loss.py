...
class WingLoss(nn.Layer):
    ...
    def forward(self, x, t, sigma=1):
        # weight = paddle.ones_like(t)
        # weight[paddle.where(t==-1)] = 0  
        # diff = weight * (x - t)
        # abs_diff = diff.abs()
        # 传入的x和t都经过乘以mask处理，这里weight就可以不要了
        abs_diff = (x - t).abs()
...
class ComputeLoss:
    ...
    def __call__(self, p, targets):  # predictions, targets
        ...
        for i, pi in enumerate(p):  # layer index, layer predictions
            ...
            if n:
                ...
                # 修改：输出形式为[xy, wh, conf, landmarks, cls]
                pxy, pwh, _, plandmarks, pcls = pi[b, a, gj, gi].split((2, 2, 1, 8, self.nc), 1)  # target-subset of predictions
                ...
                pred_prob = pi[b, a, gj, gi]
                pred_prob = F.sigmoid(pred_prob[..., 4])
                ...
                # landmarks loss
                # 训练时学习的关键点数据为ceil中心到关键点坐标距离与anchor的比值
                # 这里限制输出为（-2, 2）
        ...
        return (lbox + lobj + lcls + lmark) * bs, paddle.concat((lbox, lobj, lcls, lmark)).detach()
    ...
    def build_targets(self, p, targets):
        ...
        gain = paddle.ones([15])  # normalized to gridspace gain
        ...
        for i in range(self.nl):
            ...
            # landmarks部分
            gain[6:14] = gain[6:14] * 0 + paddle.to_tensor(shape)[[3, 2, 3, 2, 3, 2, 3, 2]]
            # Match targets to anchors
            t = targets * gain  # shape(3,n,15)
            if nt:
                # Matches
                ...
                if j.sum().item() == 0:
                    ...
                    # 没有匹配的标签时, landmarks相关列表同样添加False, 保持对齐
                    landmarks.append(False)
                    lmks_mask.append(False)
                    continue
            ...
            # Define
            bc, gxy, gwh, lks, a = t.split([2, 2, 2, 8, 1],1)  # (image, class), grid xy, grid wh, anchors
