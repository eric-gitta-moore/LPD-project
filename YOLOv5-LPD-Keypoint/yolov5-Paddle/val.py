...
from utils.general import (LOGGER, TQDM_BAR_FORMAT, Profile, check_dataset, check_img_size, check_requirements,
                           ...
                           print_args, scale_boxes, xywh2xyxy, xyxy2xywh, scale_points)
...
def process_batch(detections, labels, iouv):
    ...
    Arguments:
        detections (array[N, 14]), x1, y1, x2, y2, conf, landmarks, class
    ...
    correct_class = labels[:, 0:1] == detections[:, 13]
...
def run(
    ...
    for batch_i, (im, targets, paths, shapes) in enumerate(pbar):
        ...
        # Loss
        if compute_loss:
            loss += compute_loss(train_out, targets)[1]  # box, obj, cls, lmark

        # NMS
        if targets.shape[0]:
            targets[:, 2:6] *= paddle.to_tensor((width, height, width, height))  # box to pixels
            targets[:, 6:14] *= paddle.to_tensor((width, height, width, height, width, height, width, height))  # landmark to pixels
        ...
        for si, pred in enumerate(preds):
            ...
            predn[:, 5:13] = scale_points(im[si].shape[1:], predn[:, 5:13], shape, shapes[si][1])  # native-space pred
            # Evaluate
            if nl:
            ...
            stats.append((correct, pred[:, 4], pred[:, 5+8], labels[:, 0]))  # (correct, conf, pcls, tcls)
